...
def test_get_value_from_key_with_square_brackets():
    my_dict = {"key": "value"}
    assert get_value_from_key_with_square_brackets(my_dict, "key") == "value"
    with pytest.raises(KeyError):
        get_value_from_key_with_square_brackets(my_dict, "key2")


def test_get_value_from_key_with_get():
    my_dict = {"key": "value"}
